...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed):
        ...
        # Critic Network (w/ Target Network), initialize at Class level, shared with all actors
        ...
        if Agent.critic_optimizer is None:
            Agent.critic_optimizer = optim.Adam(
                Agent.critic_local.parameters(), lr=LR_CRITIC, weight_decay=WEIGHT_DECAY)
        ...
        # Shared Replay memory - only intitialize once per class
        if Agent.memory is None:
            print("Initializing ReplayBuffer")
    ...
    def step(self, state, action, reward, next_state, done, timestep):
        ...
        if len(Agent.memory) > BATCH_SIZE and timestep % LEARN_EVERY == 0:
    ...
    def learn(self, experiences, gamma):
        ...
        torch.nn.utils.clip_grad_norm_(
            self.actor_local.parameters(), 1)  # clip
